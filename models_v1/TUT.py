...
from models_v1.MyLayers import LearnableAPE, SinusoidalAPE, RPE, LocalAttention
...
class TUTSingleStage(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # --- Align all down‑sampled masks with the feature tensor’s device ---
        mask_down = [m.to(x.device) for m in mask_down]
        ...
        # Ensure first‑scale mask is on the same device before multiplication
        out = self.conv_out_layer(x) * mask_down[0].to(x.device).unsqueeze(1)
...
class TUT(nn.Module):
    ...
    def forward(self, x, mask):
        # Make sure the padding mask is on the same device as the feature tensor
