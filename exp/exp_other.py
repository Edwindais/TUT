...
from models_v1.MSTCN import MS_TCN, MS_TCN2
...
class ExpOther:
    ...
    def _build_model(self, model_type):
        assert model_type in ['MSTCN', 'MSTCN2', 'ASFormer']

        if model_type == 'MSTCN':
            model = MS_TCN(self.configs)
        elif model_type == 'MSTCN2':
            model = MS_TCN2(self.configs)
        elif model_type == 'ASFormer':
            model = ASFormer(self.configs)
